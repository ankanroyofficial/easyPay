...
import {
    ...
    TextInput, ActivityIndicator, Keyboard, SafeAreaView, Alert, Platform,KeyboardAvoidingView
} from 'react-native'
...
function PaymentRequiredModal({ ispress, onPress, name, imageName, taskTitle, taskSlugname, bid_id, reviewModal }) {
    const webref = useRef()
        ...
        try {
            if (payment_method != "") {
                ...
                if (res.data.redirect_url) {
                    // navigation.navigate("PaymentWebPage", { apiUrl: res.data.redirect_url })
    ...
    const getAllData = () => {
        try {
            ...
            channel.bind('question-event', ((data) => { getOfferData() }))
            channel.bind('bid-apply-event', ((data) => { getOfferData() }))
            channel.bind('bid-withdraw-event', ((data) => { getOfferData() }))
            channel.bind('bid-update-event', ((data) => { getOfferData() }))
            channel.bind('bid-reply-event', ((data) => { getOfferData() }))
            channel.bind('task-assign-event', ((data) => { getOfferData() }))
        } catch (error) {
...

                    {
                        language === "en" ?
                            ...
                            <Text style={{
                                ...
                                paddingTop: "2%",
                                paddingHorizontal: "2%"
                            }} >پرداخت شما به صورت امن انجام پذیرفت و کار به <Text style={{ fontFamily: 'roboto-medium', color: Colors.primary, fontSize: Normalize(11) }}>{nameShorting(name)}</Text> محول گردید</Text>
                    }
    ...
    const PrivateMSG = () => (
        <View style={{ backgroundColor: Colors.white, borderRadius: 4, margin: Normalize(15), }} >
            <View style={{ height: Normalize(50), width: "100%", flexDirection: language == "en" ? "row" : "row-reverse", justifyContent: "space-between", alignItems: "center", borderBottomColor: "#E8E8E8", borderBottomWidth: 1 }}>
                <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }} >
                    <Text style={{
                        fontSize: Normalize(14),
                        fontFamily: 'roboto-bold',
                        ...
                        <View style={{ flex: 1, borderRadius: 10, borderColor: "#D8D8D8", borderWidth: Normalize(1), justifyContent: "space-between", padding: "2%" }} >
                            <View style={{ flex: 1 }} >
                                <TextInput
                                    ...
                                    style={{ textAlign: language == "en" ? "left" : "right", color: '#000' }}
    ...
    return (
        ...
        >
            <KeyboardAvoidingView behavior={(Platform.OS === 'ios') ? "padding" : null} style={{ flex: 1, backgroundColor: "rgba(52,52,52,0.5)", justifyContent: "center" }} >
                {
                    modalChange == "" ?
                        amountDetails != "" ?
                            <View style={{ backgroundColor: Colors.white, borderRadius: 4, margin: Normalize(15), }} >
                                <ScrollView showsVerticalScrollIndicator={false} >
                                    ...
                                    <View style={{ paddingHorizontal: Normalize(15) }} >
                                        ...
                                        <Button
                                            ...
                                            // onPress={()=>Toast.show("Payment section in progress")}
