...
// import TaskDetails_header_button from '../../../../components/TaskDetails_header_button';
...
function TaskDetails({ navigation, route }) {
  ...
  const getData_after_similarTask = async (slug) => {
    try {
      setLoader(true)
      const res = {
        "jsonrpc": "2.0",
  ...
  return (
    <SafeAreaView style={{ backgroundColor: Colors.primary, flex: 1, flexDirection: 'column' }}>
      <KeyboardAvoidingView
        ...
        {loader || taskData == null || percentageCalculate(myBidAmount(taskData.get_bid)) == 0 ?
          <LoaderPage />
          :
          <View style={{ flex: 1 }} >
            {shareModal &&
            ...
            <View style={{ flex: 1, paddingHorizontal: Normalize(16) }} >
              ...
              >
                ...
                <View style={{ flexDirection: 'column' }}>
                  {opentab == 2 &&
                    <>
                      <View style={{ flexDirection: language == "en" ? "row" : "row-reverse", paddingVertical: Normalize(5), justifyContent: 'space-between' }}>
                        <View style={{ flex: 1, }} >
                          <Text style={[globalstyles.blue_Text, {
                            marginRight: Normalize(10), borderRadius: 10, marginLeft: Normalize(0), fontFamily: "Outfit-Medium", fontSize: Normalize(14),
                      ...
                      <Text numberOfLines={1} style={[globalstyles.plantext_Outfit_Medium, {
                      ...
                      <Text style={[globalstyles.plantext_Outfit_Medium, {
                        marginRight: Normalize(10), borderRadius: 10, marginLeft: Normalize(0),
                        paddingVertical: Normalize(5), fontSize: Normalize(11), paddingHorizontal: Normalize(10), marginTop: 20
                      }]}>
                        This task need to be done on
                      </Text>

                      <View style={{ flexDirection: language == "en" ? "row" : "row-reverse", paddingLeft: Normalize(10), marginVertical: Normalize(3) }}>
                      ...
                      {
                        taskData.must_have != null && taskData.must_have != "" &&
                        <Fragment>
                          <Text style={[globalstyles.plantext_Outfit_Medium, {
                            marginRight: Normalize(10), borderRadius: 10, marginLeft: Normalize(0),
                            paddingVertical: Normalize(5), fontSize: Normalize(11), paddingHorizontal: Normalize(10), marginTop: 20
                  ...
                  {opentab == 3 && <View style={{ height: Normalize(40), flexDirection: language == "en" ? "row" : "row-reverse", justifyContent: "space-between", marginTop: Normalize(20) }} >
                    {
                    ...
                    <>
                      <View style={{ paddingVertical: Normalize(15), width: "99%", alignSelf: "center", }}>
                        <Text style={[styles.headingservice, { marginTop: 20, textAlign: language == "pr" ? "right" : "left" }]}>
                          {strings.TASKDETAILS.QUESTION} {"("} {language == "en" ? `${allquestion.length}` : `${engToPersian(allquestion.length)}`} {")"}  </Text>
                      </View>
