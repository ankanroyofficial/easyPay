...
export default function AddMustHave({ navigation }) {
    const { RtlStyles, isRtl, language, setLanguage } = useRtlContext();
    const { must_have, setMust_Have } = useContext(myContext);
    ...
    const saveButton = () => {
        ...
        if (whatOrdo.length == 0) {
            ...
            navigation.navigate('PostTaskDetails');
        } else {
                                    ...
                                    <ScrollView showsVerticalScrollIndicator={false}  >
                                        ...
                                        </View>
                                        {
                                            second_onef().map((item, index) => (
                                                <View key={index} style={{ flexDirection: language == "en" ? "row" : "row-reverse", width: "100%", alignItems: "center", marginVertical: Normalize(2) }}>
                                                    <View style={{ height: Normalize(10), width: Normalize(11), justifyContent: "center", alignItems: "center", marginHorizontal: Normalize(6) }}>
