...
function GetItDone({ navigation }) {
  ...
  const getData = async () => {
    try {
      ...
      // console.log(data.data.result.placeholder_name)
      if (data.data.result) {
  ...
  const is_new_notification = async () => {
    try {
      const data = await axiosInstance.post('home');
      if (data.data.result) {
        const recent_noti_count =
          data.data.result.notifications_count.toString();
  ...
  const getAllData = () => {
    const pusher = new Pusher(pusherConfig.key, pusherConfig);
    let channel = pusher.subscribe('guru-tasker');
    channel.bind('task-assign-event', function (data) {
      if (data) {
        // console.log("Getitdone task assign ---------------")
        is_new_notification();
      }
    });
    channel.bind('post-task', function (data) {
      if (data) {
        // console.log("Getitdone task ---------------")
        is_new_notification();
      }
    });
    channel.bind('bid-apply-event', function (data) {
      if (data) {
        // console.log("Getitdone bid ---------------")
        is_new_notification();
      }
    });
    channel.bind('bid-withdraw-event', function (data) {
      if (data) {
        // console.log("Getitdone withdraw---------------")
        is_new_notification();
      }
    });
    channel.bind('message-event', function (data) {
      if (data) {
        // console.log("Getitdone message---------------", data.message_detail.message)
        is_new_notification();
      }
    });
  };
  ...
  const getNotificationData = async () => {
    try {
      ...
      if (data.data.result.user) {
        ...
        // console.log("data.data.result.user.point_1  -", data.data.result.user.point_1)
        // console.log("data.data.result.user.point_2  -", data.data.result.user.point_2)
        // console.log("data.data.result.user.point_3  -", data.data.result.user.point_3)
