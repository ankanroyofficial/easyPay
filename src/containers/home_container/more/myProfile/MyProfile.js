...
function MyProfile(props) {
  ...
  const { userSlugName, profileData, setProfileData,
    ...
    certificateList, setCertificateList, userId, drawerTabNaviData, setDrawerTabNaviData
  ...
  const getData = async (val) => {
    try {
      ...
      if (val == "emptyProfileDataLoader") {
        // console.log("emptyProfileDataLoader    ------------->")
        setLoader(true)
      ...
      if (data.data.result) {
        ...
        const {
          ...
          be_a_fixer_approval,
          phone, status,
          is_phone_verified, is_email_verified, wallet_balance, id, slug, unread
        } = data.data.result.user

        setDrawerTabNaviData({
          "be_a_fixer_approval": be_a_fixer_approval,
          "listing_count": (data.data.result.my_listings).length,
  ...
  const is_profilePage_Load_forEmpty = async () => {
    try {
      if (profileData == null) {
        getData("emptyProfileDataLoader")
      }
    } catch (error) {
  ...
  const profile_Refresh = async (val) => {
    try {
      if (val != undefined && val != "notScroll") {
        setEditLoader(true)
      }
      const data2 = await axios.post(`${axiosUrl.URL}get-country-language`)
      if (data2) {
      ...
      // console.log("puser Response", data.data.result.user.be_a_fixer_approval)
    ...
    channel.bind('send-notification-count', function (data) {
      if (data) {
        if (data.user_id == userId) {
          profile_Refresh("notScroll")
          // console.log("myProfile     pusher>>>", userId, " ", data, "amr msg")
        } else {
          // console.log("myProfile     pusher>>>", userId, " ", data, "loker msg")
        }
      }
    })
  }
  ...
  // console.log(profileData.get_user_to_portfolio)
                ...
                <ScrollView
                  ...
                  <View
                    ...
                    <View style={{ width: width, height: Normalize(140), alignItems: "center" }} >
                      ...
                      <View style={{
                        height: Normalize(120), width: width * 0.9, borderColor: Colors.boxBorder,
                        borderWidth: Normalize(1), borderRadius: Normalize(8), position: "absolute", top: Normalize(10),
                        overflow: "hidden"
                      }} >
                              ...
                              profileData.tot_review_as_tasker == 0 ?
                                ...
                                <View style={{ justifyContent: "center", alignItems: "center", paddingVertical: Normalize(10) }} >
                                  <View style={{ flexDirection: 'row', justifyContent: 'space-between' }} >
                                    <Pressable
                                      onPress={() => navigation.navigate("MyReview", { active: reviewSectionAbove })}
                    ...
                    <View style={styles.borderwithpadding} >
                      ...
                      {certificateList.map((item, index) => (
                       item.is_verify!="N"&& <View key={index} style={{ width: "100%", paddingVertical: Normalize(4), flexDirection: "row", alignItems: "center" }} >
                        ...
                        <FlatList
                          ...
                          renderItem={({ item, index }) => (
                            ...
                            <Pressable
                              ...
                              style={{ width: Normalize(95), height: Normalize(100), backgroundColor: Colors.grayf5, marginRight: width * 0.025, borderRadius: 8, overflow: "hidden", borderWidth: 1, borderColor: Colors.boxBorder }}>
                              ...
                              <Pressable
                                ...
                                <View style={{ height: Normalize(18), width: Normalize(18), borderRadius: Normalize(18) / 2, justifyContent: "center", alignItems: "center", backgroundColor: Colors.red_old }} >
                                  <Entypo
                                    name="cross"
                                    size={Normalize(15)}
                                    color={Colors.white}
