import { View, Text, SafeAreaView, Image, Dimensions, Pressable, PermissionsAndroid, BackHandler } from 'react-native';
...
export default function LocationSelectionPage() {
    ...
    const [currentLocation, setCurrentLocation] = useState({
        latitude: lMS_lat != "" ? lMS_lat : 1.28967,
        longitude: lMS_lng != "" ? lMS_lng : 103.85007,
        latitudeDelta: 0.015,
    ...
    useEffect(() => {
        if (lMS_location != "") {
            textRef.current?.setAddressText(lMS_location)
        }
    }, []);
    const getLocation = () => {
    ...
    const onpressSaveLocation = () => {
        if (
            ...
            setLMS_location(tempLocation.location);
            setLMS_lat(tempLocation.lat);
            setLMS_lng(tempLocation.lng);
            navigation.goBack();
        }
    };
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: Colors.primary }}>
            <View style={{ flex: 1, backgroundColor: Colors.white }}>
                <Header navigation={navigation} back name={'Select Location'} backFunc={handleBackButtonClick} />
                <View style={{ flex: 1 }}>
                    <View style={{ flex: 1 }}>
                        <MapView
                            mapType="standard"
