...
import { View, Text, TouchableOpacity, Image, ScrollView, SafeAreaView, Button, RefreshControl, Pressable, FlatList, BackHandler } from 'react-native';
...
function MyReview(props) {
  ...
  const active = props.route.params
  ...
  const { RtlStyles, language, setLanguage } = useRtlContext();
  const {
                                ...
                                renderItem={({ item, index }) => {
                                  return (
                                    <View
                                      ...
                                      <View style={{ flexDirection: language == "pr" ? "row-reverse" : "row", width: "100%" }}>
