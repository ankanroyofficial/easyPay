...
export default function DocumentUpload({ navigation }) {
    ...
    const { documentStatus, setDocumentStatus } = useContext(myContext)
    ...
    // const [front_document_decline_comment, setfront_document_decline_comment] = useState("")
    // const [back_document_decline_comment, setBack_document_decline_comment] = useState("")
    ...
    const get_Document_details = async () => {
        try {
            ...
            if (res.data.result.user.status == "Approved") {
    ...
    const onlynexttopage = () => {
        navigation.navigate("BecomeTasker_Agree")
    }
    ...
    const rejection_document_upload = async () => {
        try {
            ...
            setLoader(false)
            console.log("rejection_document_upload", error)
        }
    }
    ...
    const whichfunc = () => {
        if ((front_document_details.is_verify == "I" && back_document_details.is_verify == "I") || (front_document_details.is_verify == "I" && back_document_details.is_verify == "Y") || (front_document_details.is_verify == "Y" && back_document_details.is_verify == "I") || (front_document_details.is_verify == "Y" && back_document_details.is_verify == "Y")) {
            onlynexttopage()
        } else {
            fileuploadButton()
        }
                ...
                <CurveDesing_Component bottom>
                    <View style={globalstyles.container_only_padding} >
                        ...
                        <ScrollView showsVerticalScrollIndicator={false} >
                            ...
                            <View style={{ flex: 1 }} >
                                ...
                                <View style={{}} >
                                    ...
                                    <View style={{ paddingBottom: Normalize(8) }} >
                                        ...
                                        {
                                            front_document_details.is_verify == "N" && front_document_details.decline_comment != null && show_frontImage == "" &&
                                            <>
                                                <Text style={[globalstyles.plantext_Outfit_Medium, { fontSize: Normalize(11), color: Colors.primary, marginTop: Normalize(5), textAlign: "left" }]}>Reason :</Text>
                                                <Text style={[globalstyles.plantext_Outfit_Medium, { fontSize: Normalize(11), color: Colors.red_old, textAlign: "left" }]}>{front_document_details.decline_comment}</Text>
                                    ...
                                    <View style={{ paddingBottom: Normalize(8) }} >
                                        ...
                                        {
                                            back_document_details.is_verify == "N" && back_document_details.decline_comment != null && show_backImage == "" &&
                                            <>
                                                <Text style={[globalstyles.plantext_Outfit_Medium, { fontSize: Normalize(11), color: Colors.primary, marginTop: Normalize(5), textAlign: "left" }]}>Reason :</Text>
                                                <Text style={[globalstyles.plantext_Outfit_Medium, { fontSize: Normalize(11), color: Colors.red_old, textAlign: "left" }]}>{back_document_details.decline_comment}</Text>
                                            </>
                                        }
                                    </View>
                                </View>
                            ...
                            <Button
                                nextarrow={loader ? false : true}
                                onPress={whichfunc}
                                name={loader ? "" : whichText()}
